...
import { ref, computed, watch, watchEffect, onMounted, onUnmounted, nextTick, useTemplateRef } from 'vue'
...
const lineInfoQuery = useQuery({
  queryKey: computed(() => ['lineInfo', Number(props.lineId), Number(props.lineType)]),
  queryFn: () => getLineList({ Line_Id: Number(props.lineId), Line_Type: Number(props.lineType) }),
  enabled: computed(() => !!props.lineId && !!props.lineType),
  placeholderData: keepPreviousData,
})

const stationsQuery = useQuery({
  queryKey: computed(() => ['stations', Number(props.lineId), Number(props.lineType)]),
...
const gpsQuery = useQuery({
  queryKey: computed(() => ['gps', Number(props.lineId), Number(props.lineType)]),
  queryFn: () => getLineGPSList(Number(props.lineId), Number(props.lineType)),
  enabled: lineInfoQuery.isSuccess,
})

const scheduleQuery = useQuery({
  queryKey: computed(() => ['schedule', Number(props.lineId), Number(props.lineType)]),
  queryFn: () => getDayPlanTime(Number(props.lineId), Number(props.lineType)),
  enabled: lineInfoQuery.isSuccess,
})

const busQuery = useQuery({
  queryKey: computed(() => ['buses', Number(props.lineId), Number(props.lineType)]),
...
const selectedStationBuses = computed(() => {
  ...
  for (const bus of buses.value) {
    ...
    const time = estimateArrivalTime(bus, stations.value, selectedStation.value.Sort)
...
function getStationMarkerContent(station: StationInfo) {
  const isTerminal = station.Sort === 1 || station.Sort === stations.value.length
  const isSelected = selectedStation.value?.Station_Id === station.Station_Id
  const hasBus = buses.value.some(b => b.Current_Station_Sort === station.Sort)
  ...
  return {
    ...
    offset: new window.AMap.Pixel(isTerminal ? -12 : isSelected ? -8 : -6, isTerminal ? -12 : isSelected ? -8 : -6)
  }
}
...
watch([() => props.lineId, () => props.lineType], () => { autoSelected = false })
watchEffect(() => {
  const s = stations.value
  ...
  if (queryStation) {
    ...
    if (found) { selectedStation.value = found }
    return
  }
...
watchEffect(() => {
  if (showStationList.value && selectedStation.value && stationListEl.value) {
    nextTick(() => {
      const el = stationListEl.value?.querySelector(`[data-station-id="${selectedStation.value?.Station_Id}"]`)
...
onUnmounted(() => {
  queryClient.cancelQueries({ queryKey: ['buses', Number(props.lineId), Number(props.lineType)] })
})
...
async function initMap() {
  ...
  await new Promise<void>((resolve) => {
    ...
    script.src = 'https://webapi.amap.com/maps?v=2.0&key=95a273969365aef06b70c7fa84375718&plugin=AMap.Geolocation'
  ...
  if (userPosition.value) {
    ...
    for (const s of stations.value) {
      const d = haversineDistance(
        ...
        s.LatLng.longitude, s.LatLng.latitude
      )
      if (d < minDist) { minDist = d; nearest = s }
    }
    if (nearest && minDist < 2000) { selectedStation.value = nearest; return }
  }
...
async function switchDirection() {
  ...
  const newType = currentLine.value.Line_Type === LineType.DOWN ? LineType.UP : LineType.DOWN
  const lines = await getLineList({ Line_Id: currentLine.value.Line_Id, Line_Type: newType })
  if (lines.length > 0) {
    showStationList.value = false
    showSchedule.value = false
    hasLocated.value = false
    router.replace({ name: 'map', params: { lineId: lines[0].Line_Id, lineType: lines[0].Line_Type } })
  }
}
</script>
  ...
  <div class="flex-1 flex flex-col">
    <header class="bg-white shadow-sm z-40 flex-shrink-0">
      <div class="px-4 py-3 flex items-center gap-3">
        <button class="p-2 -ml-2 active:bg-gray-100 rounded-full" @click="goBack">
          <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <div class="flex-1 min-w-0">
          <div class="font-medium text-gray-800">{{ currentLine?.Line_Name }}</div>
          <div class="text-xs text-gray-500 truncate">{{ currentDirection }}</div>
        </div>
        <button
          v-show="currentLine"
        ...
        >
          <svg class="w-6 h-6" :class="currentLine && isFavorited(currentLine) ? 'text-yellow-400' : 'text-gray-400'" fill="currentColor" viewBox="0 0 24 24">
        ...
        >
          ...
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="font-medium text-gray-800">{{ selectedBusForInfo?.BusNo }}</div>
                <div class="text-xs text-gray-500">{{ selectedBusForInfo?.Oil_Type === 1 ? '新能源' : '燃油' }}</div>
              </div>
            </div>
            <button class="p-1 active:bg-gray-100 rounded-full" @click="selectedBusForInfo = null">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div class="bg-gray-50 rounded-xl p-3">
              ...
              <div class="font-medium text-gray-800">
                {{ stations.find(s => s.Sort === selectedBusForInfo?.Current_Station_Sort)?.Station_Name || '-' }}
              </div>
            </div>
            <div class="bg-gray-50 rounded-xl p-3">
              <div class="text-gray-500 text-xs mb-1">状态</div>
              <div class="font-medium" :class="selectedBusForInfo?.IsArrive === 1 ? 'text-green-600' : 'text-orange-500'">
    ...
    <div
      ...
      class="flex-shrink-0 bg-white shadow-lg z-20"
    >
      <!-- 到站信息 -->
      <div class="p-4">
        <div class="mb-1">
          <div class="font-medium text-gray-800">{{ selectedStation.Station_Name }}</div>
        </div>
        <div class="text-xs text-gray-400 mb-3">第{{ selectedStation.Sort }}站</div>
        ...
        <div v-if="selectedStationBuses.length > 0" class="space-y-2">
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="text-sm font-medium text-gray-800">{{ item.bus.BusNo }}</div>
                <div class="text-xs text-gray-500">还有{{ item.bus.Current_Station_Sort - selectedStation.Sort + 1 }}站</div>
              </div>
            </div>
            <div class="text-right">
              <div class="text-lg font-bold text-orange-500">约{{ item.time }}分钟</div>
            </div>
          </div>
        </div>
      ...
      <DrawerContent>
        ...
        <div ref="stationListEl" class="overflow-y-auto px-4 pb-6" style="max-height: 60vh;">
          ...
          >
            ...
            >{{ station.Sort }}</div>
            <div class="flex-1 min-w-0">
              <div class="text-sm text-gray-800 truncate">{{ station.Station_Name }}</div>
            </div>
            <div class="flex items-center gap-1.5 flex-shrink-0">
              <div
                v-for="bus in buses.filter(b => b.Current_Station_Sort === station.Sort)"
                :key="bus.BusId"
              ...
              >🚌</div>
              <div
                v-if="buses.some(b => b.Current_Station_Sort === station.Sort)"
                class="w-2 h-2 bg-red-500 rounded-full"
              ></div>
              <div
                v-if="buses.some(b => b.Current_Station_Sort < station.Sort && b.Current_Station_Sort > (stations.find(s => s.Sort === station.Sort - 1)?.Sort ?? 0))"
                class="text-xs text-orange-500 font-medium"
              >
                {{ buses.filter(b => b.Current_Station_Sort < station.Sort && b.Current_Station_Sort > (stations.find(s => s.Sort === station.Sort - 1)?.Sort ?? 0)).length }}辆途中
              </div>
            </div>
          </div>
        </div>
    ...
    <Drawer v-model:open="showSchedule">
      <DrawerContent>
        ...
        <div class="overflow-y-auto px-4 pb-6" style="max-height: 60vh;">
          <div v-if="schedule.length === 0" class="text-center text-gray-400 py-8 text-sm">
            暂无发车数据
          </div>
          <div
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-gray-800">{{ item.Plan_Time }}</div>
              <div class="text-xs text-gray-400 truncate">{{ item.Start_Station_Name }} → {{ item.End_Station_Name }}</div>
            </div>
          </div>
        </div>
      </DrawerContent>
